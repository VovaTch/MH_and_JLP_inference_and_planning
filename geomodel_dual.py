...
class GeoModel:
    ...
    def add_measurement(self, measurement, new_da_realization, graph, initial, da_realization):
        ...
        for realization in new_da_realization:
            ...
            meas_x = measurement[measurement_number][2] * np.cos(measurement[measurement_number][0] / 180 * np.pi) \
                     * np.cos(measurement[measurement_number][1] / 180 * np.pi)
            meas_y = measurement[measurement_number][2] * np.sin(measurement[measurement_number][0] / 180 * np.pi) \
                     * np.cos(measurement[measurement_number][1] / 180 * np.pi)
            meas_z = measurement[measurement_number][2] * np.sin(measurement[measurement_number][1] / 180 * np.pi)

            pose_meas = gtsam.Pose3(gtsam.Rot3.Ypr(0.0, 0.0, 0.0), np.array([meas_x, meas_y, meas_z]))

            graph.add(gtsam.BetweenFactorPose3(self.X(path_length), self.XO(realization),
                                               pose_meas, self.model_noise))
    ...
    def log_pdf_value(self, measurement, point_x, point_xo):
        ...
        aux_factor = gtsam.BetweenFactorPose3(1, 2, pose_meas, self.model_noise)
        poses = gtsam.Values()
    ...
    def model_sample(pose_x, pose_xo, noise=np.eye(6), ML_sample=False):
        ...
        if ML_sample is False:
            ...
            sampled_pose = gtsam.Pose3(gtsam.Rot3.Ypr(sampled_rel_pose_vector[2], sampled_rel_pose_vector[1],
                                                    sampled_rel_pose_vector[0]),
                               np.array([sampled_rel_pose_vector[3], sampled_rel_pose_vector[4],
                                                       sampled_rel_pose_vector[5]]))
            return [math.atan2(sampled_pose.y(), sampled_pose.x()),
                    math.atan2(sampled_pose.z(), np.sqrt(sampled_pose.x()**2 + sampled_pose.y()**2)),
                    np.sqrt(sampled_pose.x()**2 + sampled_pose.y()**2 + sampled_pose.z()**2)]
        else:
            return [math.atan2(rel_pose.y(), rel_pose.x()),
                    math.atan2(rel_pose.z(), np.sqrt(rel_pose.x()**2 + rel_pose.y()**2)),
                    np.sqrt(rel_pose.x()**2 + rel_pose.y()**2 + rel_pose.z()**2)]
