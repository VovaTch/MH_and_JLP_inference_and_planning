...
class HybridBelief:
    ...
    def add_measurements(self, geo_measurements, sem_measurements, da_current_step, number_of_samples=10,
        ...
        #print('weight vector: ' + str(self.weight_pruning_basic()))
        #print('sum: ' + str(np.sum(self.weight_pruning_basic())))
    ...
    # Planning function; generate future measurements from models, gets a list. This is for myopic. For non-myopic
    # I would have to create an additional function.
    def generate_future_measurements(self, action, action_noise, geo_noise, ML=True, ML_cls=True, ML_geo=True,
                                     pose_samples=2, measurement_samples=5, overwrite_geo=None):

        # If maximum likelihood assumption is true, generate a single geo and sem measurement per object
        if ML is True:

            # Search for maximum weight realization
            cls_vector = self.compute_cls_weights()
            highest_realization = None
            highest_weight = 0
            for realization in self.belief_matrix:
                if highest_weight <= cls_vector[realization[0]]:
                    highest_weight = cls_vector[realization[0]]
                    highest_realization = realization

            geo_meas = dict()
            sem_meas = dict()
            ML_next_pose = self.belief_matrix[highest_realization].motion_sample(action, action_noise, ML=True)
            for obj in self.obj_realization:

                # ML of estimated object pose
                ML_object_pose = self.belief_matrix[highest_realization].result.\
                    atPose3(self.belief_matrix[highest_realization].XO(obj))
                DA_flag = self.da_model.object_observation_3d(ML_next_pose, ML_object_pose)
                if DA_flag is 1 and overwrite_geo is None:
                    geo_meas[obj] = self.geo_model.model_sample(ML_next_pose, ML_object_pose, noise=geo_noise,
                                                                ML_sample=ML_geo)
                    ML_class = self.find_object_cls_in_realization(highest_realization[0], obj)
                    sem_meas[obj] = self.cls_model.model_sample(ML_next_pose, ML_object_pose, ML_class[1],
                                                                ML_sample=ML_cls)
                # Overwrite if geo measurements are given; Used for ML assumption over all pose realizations
                elif DA_flag is 1 and overwrite_geo is not None:
                    geo_meas[obj] = overwrite_geo[obj]
                    ML_class = self.find_object_cls_in_realization(highest_realization[0], obj)
                    sem_meas[obj] = self.cls_model.model_sample(gtsam.Pose3(gtsam.Pose2(0.0, 0.0, 0.0)),
                                                                geo_meas[obj], ML_class[1],
                                                                ML_sample=ML_cls)

        # If ML assumption is false, sample next pose from realizations, and then sample measurements accordingly
        # TODO:SOLVE THE REALIZATION FORMAT INCOMPATIBILITY
        else:

            # Create 2 lists: one of realization, and a second of categorical weights to sample said realization
            cls_vector = self.compute_cls_weights()
            class_probability = list(cls_vector.values())
            realizations_cls = list(cls_vector.keys())
            realizations = list()
            # From single CLS realizations bring the full realization
            for realization_cls in realizations_cls:
                for realization in self.belief_matrix:
                    if realization_cls is realization[0]:
                        realizations.append(realization)

            geo_meas = dict()
            sem_meas = dict()
            for pose_idx in range(pose_samples):

                chosen_realization_idx = np.random.choice(range(len(realizations)), p=class_probability)
                chosen_realization = realizations[chosen_realization_idx]
                sample_next_pose = self.belief_matrix[chosen_realization].motion_sample(action, action_noise, ML=False)
                for obj in self.obj_realization:

                    # Initialization if object not in measurement list
                    if obj not in geo_meas:
                        geo_meas[obj] = list()
                        sem_meas[obj] = list()

                    for measurement_idx in range(measurement_samples):

                        # samples from estimated object pose
                        sample_object_pose = self.belief_matrix[chosen_realization].result. \
                            atPose3(self.belief_matrix[chosen_realization].XO(obj))
                        DA_flag = self.da_model.object_observation_3d(sample_next_pose, sample_object_pose)

                        if DA_flag is 1:
                            geo_meas[obj].append(self.geo_model.model_sample(sample_next_pose, sample_object_pose,
                                                                        noise=geo_noise, ML_sample=False))
                            sample_class = self.find_object_cls_in_realization(chosen_realization[0], obj)
                            sem_meas[obj].append(self.cls_model.model_sample(sample_next_pose, sample_object_pose,
                                                                             sample_class[1], ML_sample=ML_cls))

        return geo_meas, sem_meas
    ...
    # Clone function, required when we planning to account for different action sequences
    def clone(self):

        new_object = copy.copy(self)
        for realization in self.belief_matrix:
            new_object.belief_matrix[realization] = self.belief_matrix[realization].clone()
        new_object.obj_realization = self.obj_realization.copy()
    ...
    def graph_realizations_in_one(self, key_num, fig_num=0, show_obj=False,
                                  show_weights=False, show_plot=True, plt_save_name=None, show_elipses=True,
                                  plot_line=False, color='r', single_plot=False, show_robot_pose=True, ax=None,
                                  enlarge=1):
        ...
        for realization in self.belief_matrix:
            ...
            if self.belief_matrix[realization].logWeight >= log_weight_comparison:
                log_weight_comparison = self.belief_matrix[realization].logWeight
                max_realization = realization

            if show_obj is True and single_plot_flag is False:

                # If single plot is activated, only the highest weight realization will appear.
                ...
                for obj in self.obj_realization:
                    ...
                    plotterdaac2d.plotPoint2(fig_num, [pose_obj.x(), pose_obj.y()], 'go', ax=ax_first)
                    ax.text(pose_obj.x(), pose_obj.y(), 'O' + str(obj))
                    ...
                    if show_elipses is True:
                        if show_weights is False:
                            ...
                            #                            alpha=1, enlarge=3,
                            #                            linewidth=line_thickness)
                            plotterdaac2d.plot_ellipse(ax, [pose_obj.x(), pose_obj.y()], pose_obj_cov_rotated[3:5,3:5], enlarge=enlarge)
                        else:
        ...
        # if show_obj is True:
        #     for idx in self.obj_realization:
        #         pose_xo = self.belief_matrix[max_realization].result.atPose3(self.belief_matrix[max_realization].XO(idx))
        #         pose_xo_cov = self.belief_matrix[max_realization].marginals_after.marginalCovariance(self.belief_matrix[max_realization].XO(idx))
        #         plotterdaac2d.plotPoint2(fig_num, pose_xo.translation(), 'go')
        #
        #         # Annotate the point
        #         ax.text(pose_xo.x(), pose_xo.y(), 'O' + str(idx))
        #
        #         # Plot covariance ellipse
        #         mu, cov = plotterdaac2d.extract_mu_cov_2d(self.belief_matrix[max_realization].result, self.belief_matrix[max_realization].marginals_after, self.belief_matrix[realization].XO(idx))
        #
        ...
        for realization in self.belief_matrix:
            if plot_line is True:
                ...
                while self.belief_matrix[realization].result.exists(self.belief_matrix[realization].X(idx)):
                    pose_x_collector.append(self.belief_matrix[realization].result.atPose3(self.belief_matrix[realization].X(idx)))
