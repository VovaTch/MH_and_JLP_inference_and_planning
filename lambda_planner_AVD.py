...
class LambdaPlannerPrimitives:
    ...
    def __init__(self, lambda_belief, AVD_mapping, AVD_poses, entropy_lower_limit=None, ):
    ...
    def planning_session(self, action_noise, geo_noise, current_location_name, horizon=1, ML_planning=True, motion_primitives=None,
            ...
            for prim_idx in range(len(function_motion_primitives)):
                ...
                for ind_belief in range(len(candidate_lambda_belief.belief_list)):
                    ...
                    for key in geo_meas_ind:
                        ...
                        rotation_avg[key] = gtsam.Rot3.Ypr(0.0, 0.0, 0.0)
                        meas_x = geo_meas_ind[key][2] * np.cos(geo_meas_ind[key][0] / 180 * np.pi) \
                                 * np.cos(geo_meas_ind[key][1] / 180 * np.pi)
                        meas_y = geo_meas_ind[key][2] * np.sin(geo_meas_ind[key][0] / 180 * np.pi) \
                                 * np.cos(geo_meas_ind[key][1] / 180 * np.pi)
                        meas_z = geo_meas_ind[key][2] * np.sin(geo_meas_ind[key][1] / 180 * np.pi)
                        geo_meas_ind_avgpart = gtsam.Pose3(gtsam.Rot3.Ypr(0.0, 0.0, 0.0),
                                                           np.array([meas_x / len(candidate_lambda_belief.
                                                                                               belief_list),
                                                                     meas_y / len(candidate_lambda_belief.
                                                                                               belief_list),
                                                                     meas_z / len(candidate_lambda_belief.
                                                                                               belief_list)]))
