...
def motion_primitive_planning(seed, Lambda_BSP_mode, reward_mode, cls_model, number_of_beliefs,
                              ...
                              initial_image = '100110000010101.jpg'):
        ...
        for idx in range(number_of_beliefs):
            ...
            belief_list.append(
                HybridBelief(5, geo_model, da_model, cls_model, cls_prior_rand, GT_poses[0],
                             prior_noise,
                             cls_enable=cls_enable, pruning_threshold=5, ML_update=ML_update))
    ...
    #                              plt_save_name=None, pause_time=None, limitx=[-10, 10], limity=[-10, 10],
    ...
    if Lambda_BSP_mode == 1:
        plt_ax = lambda_belief.belief_list[0].graph_realizations_in_one(idx + 1, fig_num=0, show_obj=True,
                                                                    ...
                                                                    show_elipses=False)
    if Lambda_BSP_mode == 2:
        plt_ax = lambda_belief.display_graph(plot_line=True, display_title=False, show_plot=False)

    plt_ax.set_xlim(-8, 15)
    plt_ax.set_ylim(-10, 10)

#    lambda_belief.lambda_bar_error_graph(show_plt=False, ax=bar_ax, color=color_bar, width_offset=width_offset, GT_bar=False)
    plt.tight_layout()
...
def main():
    ...
    plt.xlim(0, 20)
    plt.legend(loc='lower left')
    fig.savefig('../figures/Plan_Exp_JLP_R1.png')
    plt.show()
    ...
    if activate_MH is True:
        ...
        # plt.ylim(0, 0.4)
        plt.xlim(0, 20)
        fig.savefig('../figures/Plan_Exp_MSDE.png')
        plt.show()
    ...
    # plt.ylim(0, 0.4)
    plt.xlim(0, 20)
    fig.savefig('../figures/Plan_Exp_MSDE_JLP.png')
    plt.show()
    ...
    plt.tight_layout()
    plt.xlim(1, 20)
    fig.savefig('../figures/Plan_Time_Exp.png')
    plt.show()
