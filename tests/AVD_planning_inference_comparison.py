...
def motion_primitive_planning(seed, Lambda_BSP_mode, reward_mode, cls_model, number_of_beliefs,
    ...
    # Load json file
    actions = ['forward','rotate_cw','rotate_ccw','backward','left','right']
    ...
    class_GT = {15: 2, 4: 3, 14: 2, 3: 2, 17: 3, 19: 2, 20: 2, 23: 5, 29: 2, 2: 2, 5: 2}
    ...
    if Lambda_BSP_mode == 1:
        plt_ax = lambda_belief.belief_list[0].graph_realizations_in_one(idx + 1, fig_num=0, show_obj=True,
                                                                    ...
                                                                    enlarge=3)
    if Lambda_BSP_mode == 2:
        plt_ax = lambda_belief.display_graph(plot_line=True, display_title=False, show_plot=False, color=color_bar,
                                             ax=plt_ax, enlarge=3)
...
def main():
    ...
    ax_bar_r1.legend(['WEU', 'JLP', 'JLP', 'MH', 'MH'], bbox_to_anchor=(0.5, 1), loc='lower center',
                     ncol=5)
    ...
    if activate_MH is True:
        ...
        for idx in range(number_of_seeds):
            # plt.plot(JLP_entropy_R1[idx], color='blue', alpha=1 / number_of_seeds, label='JLP')
            plt.plot(MH_entropy_R1[idx], color='red', alpha=1 / number_of_seeds, label='MH')
        ...
        fig.savefig('../figures/AVD_Inf_MH_R1.png')
        plt.show()
    ...
    if activate_MH is True:
        ...
        for idx in range(number_of_seeds):
            # plt.plot(JLP_entropy_R1[idx], color='blue', alpha=1 / number_of_seeds, label='JLP')
            plt.plot(MH_e_entropy_R1[idx], color='red', alpha=1 / number_of_seeds, label='MH')
            # plt.plot(JLP_entropy_R2[idx], color='blue', alpha=1 / number_of_seeds, label='JLP')
        ...
        plt.xlim(0, 20)
        plt.legend(loc='lower left')
        fig.savefig('../figures/AVD_Inf_MH_R2.png')
        plt.show()
    ...
    for idx in range(number_of_seeds):
        plt.plot(JLP_entropy_R1[idx], color='blue', alpha=1 / number_of_seeds, label='JLP')
        # plt.plot(-MH_entropy_R1[idx], color='red', alpha=1 / number_of_seeds, label='MH R1')
    ...
    plt.xlim(0, 20)
    plt.legend(loc='lower left')
    fig.savefig('../figures/AVD_Inf_JLP_R1.png')
    plt.show()

    # R2 comparison JLP
    fig = plt.figure(109)
    for idx in range(number_of_seeds):
        plt.plot(JLP_e_entropy_R1[idx], color='blue', alpha=1 / number_of_seeds, label='JLP')
        # plt.plot(-MH_entropy_R1[idx], color='red', alpha=1 / number_of_seeds, label='MH R1')
    ...
    plt.xlim(0, 20)
    plt.legend(loc='lower left')
    fig.savefig('../figures/AVD_Inf_JLP_R2.png')
    plt.show()
    ...
    plt.grid(True)
    plt.xlim(0, 20)
    fig.savefig('../figures/AVD_Stat_Inf_JLP_R2.png')
    plt.show()
    ...
    if activate_MH is True:
        ...
        plt.grid(True)
        plt.xlim(0, 20)
        fig.savefig('../figures/AVD_Stat_Inf_MH_R1.png')
        plt.show()
        ...
        plt.grid(True)
        plt.xlim(0, 20)
        fig.savefig('../figures/AVD_Stat_Inf_MH_R2.png')
        plt.show()
    ...
    if activate_MH is True:
        fig = plt.figure(200)
        for idx in range(number_of_seeds):
            plt.plot(JLP_MSDE_R1[idx], color='blue', alpha=1 / number_of_seeds, label='JLP')
            plt.plot(MH_MSDE_R1[idx], color='red', alpha=1 / number_of_seeds, label='MH')
        ...
        # plt.ylim(0, 0.4)
        plt.xlim(0, 20)
        fig.savefig('../figures/AVD_Inf_MSDE.png')
        plt.show()
    ...
    if activate_MH is True:
        ...
        plt.grid(True)
        plt.xlim(0, 20)
        fig.savefig('../figures/AVD_Inf_MSDE_R2.png')
        plt.show()
    ...
    # plt.ylim(0, 0.4)
    plt.xlim(0, 20)
    fig.savefig('../figures/AVD_Inf_MSDE_JLP.png')
    plt.show()
    ...
    if activate_MH is True:
        ...
        # plt.ylim(0, 0.4)
        plt.xlim(0, 20)
        fig.savefig('../figures/AVD_Inf_MSDE_MH.png')
        plt.show()

    if activate_MH is True:
        fig = plt.figure(400)
        idx_time = 0
        key_color = {'JLP_R1': 'blue', 'MH_R1': 'red', 'JLP_R2': 'blue', 'MH_R2': 'red', 'WEU': 'green'}
    ...
    fig = plt.figure(500)
    if activate_MH is True:
        plt.plot(np.array(range(len(JLP_entropy_exp_R1))), time_dict_track['MH_R1'], color='red', label='JLP')
        plt.text(len(time_dict_track['MH_R1']) - 2, time_dict_track['MH_R1'][-1], 'MH')
    plt.plot(np.array(range(len(JLP_entropy_exp_R1))), time_dict_track['JLP_R1'], color='blue', label='MH')
    plt.text(len(time_dict_track['JLP_R1']) - 2, time_dict_track['JLP_R1'][-1] - 1, 'JLP')
    ...
    plt.tight_layout()
    plt.xlim(1, 20)
    fig.savefig('../figures/AVD_Inf_Time.png')
    plt.show()
