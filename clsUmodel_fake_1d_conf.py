...
import numpy as np
import gtsam
import joint_lambda_pose_factor_fake_1d_conf
...
class JLPModel:
    ...
    def __init__(self):
        ...
        joint_lambda_pose_factor_fake_1d_conf.initialize_python_objects()
    ...
    def add_measurements(self, measurement_exp, measurement_cov, new_da_realization, graph, initial, da_realization):
        ...
        for realization in new_da_realization:
            ...
            graph.add(joint_lambda_pose_factor_fake_1d_conf.JLPFactor(self.X(path_length), self.XO(realization),
            ...
            if initial.exists(self.Lam(lam_realization_prev)) is False:
                initial.insert(self.Lam(lam_realization_prev), np.array([0.]))

            if initial.exists(self.Lam(lam_realization)) is False:
                initial.insert(self.Lam(lam_realization), np.array([0.]))
    ...
    @staticmethod
    def flatten_cov(covariance_matrix, matrix_len=1):
    ...
    def log_pdf_value(self, measurement_exp, measurement_cov, point_x, point_xo, lambda_prev, lambda_cur):
        ...
        """
        aux_factor = joint_lambda_pose_factor_fake_1d_conf.JLPFactor(1, 2, 3, 4, measurement_exp,
    ...
    def sample_lambda_model(self, chosen_cls, relative_position):
        ...
        if chosen_cls == 1:
            output_list = joint_lambda_pose_factor_fake_1d_conf.AuxOutputsJLP.net_output_1(np.array(relative_position))
        elif chosen_cls == 2:
            output_list = joint_lambda_pose_factor_fake_1d_conf.AuxOutputsJLP.net_output_2(np.array(relative_position))

        exp = [output_list[0]]
        rinf = np.array([[output_list[1]]])
