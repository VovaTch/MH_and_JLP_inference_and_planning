...
class JLPModel:
    ...
    def __init__(self, exp_add_1, exp_add_2, exp_add_3, rinf_add_1, rinf_add_2, rinf_add_3):
    ...
    def add_measurements(self, measurement_exp, measurement_cov, new_da_realization, graph, initial, da_realization):
        ...
        for realization in new_da_realization:
            ...
            graph.add(joint_lambda_pose_factor_2d.JLPFactor(self.X(path_length), self.XO(realization),
                                                            ...
                                                            measurement_exp[measurement_number],
                                                            np.array(deploy_matrix)))
    ...
    def sample_lambda_model(self, chosen_cls, relative_position):
        ...
        if chosen_cls == 1:
            output_list = joint_lambda_pose_factor_2d.AuxOutputs.net_output_1(np.array(relative_position))
        elif chosen_cls == 2:
            output_list = joint_lambda_pose_factor_2d.AuxOutputs.net_output_2(np.array(relative_position))
