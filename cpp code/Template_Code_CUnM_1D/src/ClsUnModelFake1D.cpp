...
std::vector<double> PythonInterpreterWrapper::ClassifRegressor::net_output_1(std::vector<double> relative_position)
{
    ...
    std::vector<double> cls_output_vector = {0.5 * cos(2 * psi + 2 * theta) + 0.5};
...
PythonInterpreterWrapper::RRegressor::net_output_R1(std::vector<double> relative_position)
{
    ...
    psi = angle_fix(psi);
    theta = angle_fix(theta);
    //std::vector<std::vector<double> > cls_output_matrix = {{K * (0.8 + 0.2 * cos(2 * (psi + theta) + 3.141))}};
...
std::vector<std::vector<double> > cholesky_2x2(std::vector<std::vector<double> > matrix)
{
    ...
    matrix_output[0][1] = matrix[0][0] / matrix_output[0][0];
    matrix_output[1][1] = std::sqrt(matrix[1][1] * matrix[1][1] - matrix_output[0][1] * matrix_output[0][1]);
                                                ...
                                                std::vector<double> relative_position, double psi2earth, double theta2earth, int candidate_class) const
{
    ...
    std::vector<double> output_d_psi = classif_regressor->expectation(relative_position_d_psi, candidate_class);
    std::vector<std::vector<double> > output_ppsi_cov = r_regressor->covariance_matrix(relative_position_d_psi, candidate_class);
...
//    std::vector<std::vector<double> > output_ppsi_r_inv = inverse_2x2(output_ppsi_r);
    ...
    std::vector<double> output_d_theta = classif_regressor->expectation(relative_position_d_theta, candidate_class);
    std::vector<std::vector<double> > output_ptheta_cov = r_regressor->covariance_matrix(relative_position_d_theta, candidate_class);
...
//    std::vector<std::vector<double> > output_py_r_inv = inverse_2x2(output_py_r);
    ...
    std::vector<double> output_pz = classif_regressor->expectation(relative_position_dz, candidate_class);
    std::vector<std::vector<double> > output_pz_cov = r_regressor->covariance_matrix(relative_position_dz, candidate_class);
...
//        double d_theta_d_zr = d_theta_d_x * rot2earth[0][2] + d_theta_d_y * rot2earth[1][2] + d_theta_d_z * rot2earth[2][2];
        ...
        for (int i = 0; i < R_dim; i++)
        {
            H_1_num[i][1] = dot_multiplication(R_change_theta[i], output) * inf_der_slow;
            H_2_num[i][1] = ( d_f_d_theta[i] - dot_multiplication(R_change_theta[i], prediction_error) ) * inf_der_slow;
            H_1[i][1] = H_1_num[i][1];
            H_2[i][1] = H_2_num[i][1];

            H_1_num[i][2] = dot_multiplication(R_change_psi[i], output) * inf_der_slow;
