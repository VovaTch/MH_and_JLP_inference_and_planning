...
import libNewFactorFake_mu3

class ClsModel:

    def __init__(self):
        # Initializing the classifier models, one per class
        self.ClsModelCore = [libNewFactorFake_mu3.ClsModelFake1(), libNewFactorFake_mu3.ClsModelFake2()]
    ...
    def add_measurement(self, measurement, new_da_realization, graph, initial, da_realization, class_realization):
        ...
        for obj in new_da_realization:
            ...
            graph.add(libNewFactorFake_mu3.ClsModelFactor3(self.X(path_length), self.XO(obj),
    ...
    def log_pdf_value(self, measurement, cls, point_x, point_xo):
        ...
        aux_factor = libNewFactorFake_mu3.ClsModelFactor3(1, 2, self.ClsModelCore[cls - 1], list(measurement))
        poses = gtsam.Values()
    ...
    def log_pdf_value_new(self, measurement, cls, point_x, point_xo):
        ...
        measurement_np = np.array(measurement)

        [expectation, covariance] = self.model_output(point_x, point_xo, cls)

        mul_temp = np.matmul(expectation - measurement_np, np.linalg.inv(covariance))
        mul_comp = np.dot(mul_temp, expectation - measurement_np)
